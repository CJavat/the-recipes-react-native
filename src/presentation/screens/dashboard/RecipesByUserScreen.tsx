...
export const RecipesByUserScreen = () => {
  const {id} =
    useRoute<RouteProp<DashboardStackParams, 'RecipesByUser'>>().params;
  const {isDark} = useThemeStore();
  const {myFavorites, getRecipesByUser} = useRecipeStore();
  ...
  useFocusEffect(
    useCallback(() => {
      setData([]);
      fetchRecipeByUser(id, limit, offset);
    }, [id, offset]),
  );
  ...
  const fetchRecipeByUser = async (
    ...
    try {
      ...
      setUserName(
        `${resp.recipes.at(0)?.User.firstName} ${
          resp.recipes.at(0)?.User.lastName
        }`,
      );
